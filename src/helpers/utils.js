export const useUtils = () => {
    ...
    const didLoadAllImages = (querySelector) => {
        ...
        images.forEach((img) => {
            // Consider image loaded if complete, even if errored (naturalWidth === 0)
    ...
    /**
     * Enhanced preloader function with timeout
     * @param {Function} onComplete - Callback when preloader should hide
     * @param {number} minTime - Minimum time to show preloader (ms)
     * @param {number} maxTime - Maximum time to wait for images (ms)
     * @param {string} querySelector - Image selector
     */
    const handlePreloader = (onComplete, minTime = 2000, maxTime = 5000, querySelector = 'img') => {
        ...
        const checkImages = () => {
            ...
            const imagesLoaded = didLoadAllImages(querySelector);
            
            if (imagesLoaded && elapsedTime >= minTime) {
                complete();
            } else if (elapsedTime >= maxTime) {
                // Force complete after max time regardless of image load status
                complete();
            } else {
    ...
    return {
        ...
        limitTextSize,
        didLoadAllImages,
        handlePreloader, // New enhanced function
        formatDate,
        formatDateInterval,
        getYearsPassedSince,
        isUrlExternal,
